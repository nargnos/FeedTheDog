...
public:
	...
	ssize_t Write(void* buf, size_t nBytes) const;
	...
	bool GetSocketOption(SocketLevel level, OptName opt, void* optval, socklen_t* len) const;
...
public:
	...
	bool Listen() const;
	int Accept(sockaddr_in& addr, socklen_t& len) const;
	int Accept() const;
...
public:
	...
	ssize_t SendTo(void* buf, size_t nBytes, int flags, const sockaddr_in& addr) const;
	ssize_t RecvFrom(void* buf, size_t nBytes, int flags, sockaddr_in* addr, socklen_t* size) const;
};
// addr参数在函数中转换字节序
