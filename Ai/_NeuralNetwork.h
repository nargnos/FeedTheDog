...
// 构建的时候可以以层为单位，用add拼接在一起，每个层甚至每个元可以定义各自的函数
// 应该能像搭积木一样组合网络，另外最好把数据放一起，否则载入gpu不好弄

// FIX: amp可以改成与cpu共享内存的方式
// FIX: 激活函数和计算梯度的函数要分出来
...
// FIX: 不应该由这个结构保存速率和上次误差这种信息
// TODO: 之后要扩展用遗传来算权的方式，看一下是否解耦完全
...
public:
	...
	{
		using namespace std;
		// 注意，这里限定Transform必须在写之前读取内容，因为输入输出都用了同一个buffer
		vector<FloatingPoint> buffer(input);
		buffer.resize(maxNeuralCount_);

		for (auto& layer : hiddenLayer_)
		{
			layer.Transform(buffer, buffer);
		}
		outputLayer_.Transform(buffer, buffer);
		return _STD vector<FloatingPoint>(begin(buffer), begin(buffer) + outputLayer_.GetNeuralCount());
	}
